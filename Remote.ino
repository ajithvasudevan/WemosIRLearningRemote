 ...
 *     $ mosquitto_sub -h 10.20.0.253 -t ir_server/ack
 *
...
// An IR detector/demodulator is connected to GPIO pin 2 (=D4) on a NodeMCU board.
#define RECV_PIN 2

#define LEARN_MODE_PIN D8
...
#define MQTT_SERVER "172.20.1.120"  // <=- CHANGE ME
...
void sendIR(const char* IR_TYPE, const char*  CODE, int BITS, int REPEATS) {
  uint64_t data = 0;
  String data_str = "";
  int ir_type = 3;  // Default to NEC codes.
  uint16_t nbits = 0;
  ...
  ir_type = atoi(IR_TYPE);
  data = getUInt64fromHex(CODE);
  nbits = BITS;
  repeat = REPEATS;
  debug("New code received via IR");
  sendIRCode(ir_type, data, CODE, nbits, repeat);
}
...
void setup(void) {
  ...
  // Setup an end-point to accept remote, button name, key and value mode
...
void loop(void) {
  ...
  if (!mqtt_client.connected()) {
    ...
    if (now - lastReconnectAttempt > MQTT_RECONNECT_TIME) {
      ...
      if (reconnect()) {
        lastReconnectAttempt = 0;
        if (boot) {
          mqtt_client.publish(MQTTack, ("IR Server just booted. IP Address: " + WiFi.localIP().toString()).c_str());
          boot = false;
        } else {
  ...
  learnMode = digitalRead(LEARN_MODE_PIN);
  //Serial.print("LearnMode = ");
...
//    output = output + resultToHumanReadableBasic(&results);
    ...
    for (uint16_t i = 1; i < results.rawlen; i++) {
      ...
      if (i < results.rawlen - 1)
        rawData += ",";  // ',' not needed on the last one
